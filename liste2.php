...
ob_start();
 ...
 header("location:liste2.php?Empty=Entrer les informations requis");
}
else { 
 $a=$_POST['nom'];
  $sql1=" SELECT * FROM responsable WHERE cni='$a' ";
    $result1 = mysqli_query($con,$sql1); 
                   
                    if (mysqli_num_rows($result1) != 1)
                        { 
                          header("location:liste2.php?Dèja=cni n'existe pas");
                                
                        }
else { 
   $a=$_POST['nom'];
  $sql=" SELECT * FROM responsable  WHERE cni='$a'  ";
  ...
  $result = mysqli_query($con,$sql);
    ...
    // $c= $_POST['numero'];
      $c=$_POST['sup'];
    $sql1 = "DELETE FROM responsable  WHERE cni='$a'";
   $resultat1 = mysqli_query($con, $sql1); 
   if ($resultat1 == FALSE) {header("location:liste2.php?Complete= Echec de l'exécution de la requête.");} 
   else { header("location:liste2.php?Complete= Votre traitment est bien effectué "); }
 }
?>
...
<html lang="en">
  <head>
    <title>Chefs | Modification</title>
    <meta charset="utf-8">
...
<div class="wrapper">
  <div class="contact-form">
    <div class="input-fields"> 
                <input type="text"  class="input"  name="nom" placeholder="Cni"  >
...
echo ' 
 ...
 ?>
  ...
  <?php 
        ...
        <?php
       $tab =array ("cni",'nomres','prenomres','dep');
  ...
  if (isset($_POST[''.$i.''])) { $a= $_POST[''.$i.''];
      $sql=" SELECT * FROM responsable WHERE id='$a' ";
...
<th>';
if ($i == '0'){if ( $row1 = mysqli_fetch_array($result1)){ echo '<input type="text" value="'.$row1[$tab[$i]].'"  class="input"  class="input" name="update" pattern="[A-Z]{2}[0-9]{5,6}">';}}
if($i == '3'){
  echo '<input type="radio" name="update"   value="GI"  >&nbsp;&nbsp;GI&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
while ( $row1 = mysqli_fetch_array($result1)){  
 echo '<input type="text"  class="input" name="update"    pattern="[A-Z ]*" value="'.$row1[$tab[$i]].'" >'; }}

echo '</th><th align="right" width="1%" style="background-color: #C0E9E7" >Confirmer &nbsp;<input type="checkbox" value="'.$a.'" name="eh"></th></tr>
...
//echo $tab[$i];
 ...
 if (isset($_POST['en'])){   
  ...
  $_POST['en'] ; 
          $sqlr=" SELECT * FROM responsable WHERE  id='".$_POST['eh']."' ";
  $sql="UPDATE responsable SET ".$_POST['en']."='".$_POST['update']."'  WHERE  id='".$_POST['eh']."' ";     
...
<div class="table-responsive">
  ...
  <tr>  <th width="10%" >Cin</th>
   <td width="10%" >'.$row["cni"].'</td>
<th align="right" width="1%" > <button name="0" value="'.$n.'"> 
 <img style="width: 12px; height: 12px" src="images/modifier.png" ></button></th></th>      </tr>     
    ...
    <th style="background-color: #C0E9E7"  >'.$row["nomres"].'
  <th align="right" width="1%" style="background-color: #C0E9E7"> <button name="1" value="'.$n.'"> 
 <img style="width: 12px; height: 12px" src="images/modifier.png" ></button></th></th></tr>

  <tr>  <th>Prenom</th>  <td>'.$row["prenomres"].'
  <th align="right" width="1%"  > <button name="2" value="'.$n.'"> 
 <img style="width: 12px; height: 12px" src="images/modifier.png" ></button> </th>                                          </td></tr>     
  ...
  <th align="right" width="1%" style="background-color: #C0E9E7"> <button name="3" value="'.$n.'"> 
 <img style="width: 12px; height: 12px" src="images/modifier.png" ></button></th></th></tr>
  ...
  <div id="myModal" class="modal">
        <div class="modal-content">
          <span class="close" id="close" style="cursor: pointer;">&times;</span>
              ...
              <p style="color: white" align="center">numero d'etudiant<br />
                
           
<input style="border: 0; border-radius: 24px; border: 2px solid #191919;  border-radius: 24px; border: 2px solid #191919; text-align: center; display: block; margin: 20px auto; width: 200px; "    type="text" name="numero">
<input style="border: 0; border-radius: 24px; border: 2px solid #191919;  border-radius: 24px; border: 2px solid #191919; text-align: center; display: block; margin: 20px auto;  width: 200px;   cursor: pointer" type="submit" name="supprimer" value="envoyer">
            </form>
        </div>
